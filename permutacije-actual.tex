...
\begin{proof}
Naj bo $\sigma$ permutacija, ki ohranja konvergenco. Potem je njeno zaporedje števil blokov $\{b_n\}_{n=1}^{\infty}$ omejeno, torej obstaja njegova limita $\lim_{n \to \infty}b_n = C\in \R$. Naj bo $\tilde{S}(n)=\sum_{k=1}^{n}a_{\sigma(k)}$ neka delna vsota vrste $\sum_{k=1}^{\infty}a_{\sigma(k)}$. Po enakosti \ref{eq:5} velja: $$\tilde{S}(n)=S(d_1^n)+\sum_{i=2}^{b_n}(S(d_i^n)-S(c_i^n-1))\leq S(d_1^n)+C(S(d_i^n)-S(c_i^n-1)).$$ Vidimo, da je $\tilde{S}(n)$ linearna kombinacija $B=2*C+1$ delnih vsot $S(i)$ prvotne vrste.
\end{proof}
